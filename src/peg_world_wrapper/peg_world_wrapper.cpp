     ...
     socket_link_box_name(socket_link_box_name)
{
    ...
    world_publisher = std::shared_ptr<ww::Publisher>(new ww::Publisher("visualization_marker",&nh,&world_wrapper));
...
void Peg_world_wrapper::initialise_table_wall(const std::string table_link_name){
    ...
    world_wrapper.wrapped_objects.push_back_box(wobj::WBox(table_link_name,dim,origin,orientation));
}
